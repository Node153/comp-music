...
// 피드백 페이지 "내 기여도 + 랭킹"(0069) — 오른쪽 탭 카드(FeedbackSidePanel)의 "기여 랭킹" 탭 내용. 단계/뱃지 없이 점수와 순위만 — 약간의 경쟁으로
...
export function ContributionPanel({
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="rounded-lg bg-main-gray p-3">
        <div className="flex items-baseline gap-1.5">
          <TrophyIcon className="h-4 w-4 self-center text-black" />
          <span className="text-2xl font-bold tabular-nums text-black">{mine.month_points}</span>
          <span className="text-xs text-active-gray">점 · 이번 달</span>
          <span className="ml-auto text-xs text-active-gray">누적 {mine.total_points}점</span>
        </div>
        <p className="mt-1 text-sm font-semibold text-black">{headline(mine)}</p>
      </div>

      <div className="flex items-center justify-between gap-2">
        <span className="text-xs font-semibold text-black">순위</span>
      ...
      {showBoard ? (
        <ol className="flex flex-col gap-1">
          ...
          {(!myRow || myRow.rank > TOP_N) && myRank && myPoints > 0 && !hidden && (
        ...
        <p className="rounded-lg bg-main-gray py-6 text-center text-xs text-active-gray">
          참여한 회원이 {MIN_PARTICIPANTS}명 이상 모이면 랭킹이 열려요
        </p>
      )}

      <div className="flex items-start justify-between gap-3 border-t border-main-gray pt-2 text-[11px] text-active-gray">
