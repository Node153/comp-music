    ...
    if (post) {
      ...
      if (
        ...
        await sendEmail(
          ...
          `<p><b>${escapeHtml(kicker?.nickname ?? "누군가")}</b>님이 일주일에 한 번뿐인 Kick을 「${escapeHtml(postTitle)}」에 줬어요.</p><p><a href="${APP_URL}/feed?feed=completion&post=${postId}&src=email_kick#${postId}">게시물 보러 가기</a></p>`,
        );
      }
    }
  } catch (err) {
