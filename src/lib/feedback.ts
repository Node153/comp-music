...
// 페이지, 알림 패널이 같이 쓴다(0062 category, 0063 status). 라벨은 글자만 — 아이콘은
// components/FeedbackIcons.tsx의 선 아이콘을 화면에서 붙인다(이모지는 브라우저마다 달라 보여서).
...
export const FEEDBACK_CATEGORIES: { value: FeedbackCategory; label: string; placeholder: string }[] = [
  ...
  { value: "praise", label: "좋아요", placeholder: "마음에 들었던 점을 알려주세요" },
];
