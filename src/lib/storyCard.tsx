...
export async function loadStoryPost(postId: string): Promise<StoryPost | null> {
  ...
  const { data: post } = await supabase
    .from("posts")
    .select("id, user_id, title, caption, media_type, image_url, audio_url, video_url, thumbnail_url, visibility, status")
    .eq("id", postId)
    .maybeSingle();
...
export async function renderStoryCard(post: StoryPost, { withTrack = false } = {}) {
  ...
  const [cover, fontBold, fontMedium] = await Promise.all([
    ...
    loadFont(800, post.title),
    loadFont(500, `${post.authorName}@${cta}${footer}DEMO ON Compmusic♫▶`),
  ]);
    ...
    (
      ...
      >
        ...
        >
          ...
          {post.authorName && (
            <div style={{ display: "flex", marginTop: 20, fontSize: 40, fontWeight: 500, color: "#b8b8bc" }}>
              @{post.authorName}
            </div>
          )}
          <div
